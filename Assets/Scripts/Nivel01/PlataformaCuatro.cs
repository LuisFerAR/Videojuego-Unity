...
public class PlataformaCuatro : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if(transform.position.y > GameManager.supDer.y -3 && direccion.y >0)
        {
            direccion.y = -direccion.y;
        }
        //restriccion de limite inferior
        if(transform.position.y < GameManager.infIzq.y + 1  && direccion.y < 0)
        {
            direccion.y = -direccion.y;
        }
    }
